...
namespace NBTExplorer.Windows
{
    ...
    public partial class MainForm : Form
    {
        ...
        void _menuItemOpenInExplorer_Click(object sender, EventArgs e)
        {
            if (_nodeTree.SelectedNode.Tag is DirectoryDataNode) {
                ...
                try {
                    ...
                    MessageBox.Show(ex.Message, "无法打开目录！", MessageBoxButtons.OK, MessageBoxIcon.Error);
                }
            }
        }
        ...
        {
            if (!ConfirmAction("你向打开新文件吗？"))
                return;

            using (OpenFileDialog ofd = new OpenFileDialog() {
                RestoreDirectory = true,
                Multiselect = true,
                Filter = "所有文件类型|*|NBT 文件 (*.dat, *.schematic)|*.dat;*.nbt;*.schematic|区块文件 (*.mca, *.mcr)|*.mca;*.mcr",
                FilterIndex = 0,
            }) {
        ...
        private void OpenPaths (string[] paths)
        {
            ...
            if (failCount > 0) {
                MessageBox.Show("无法打开一个或多个选择的文件！\n" +
                                  "解决方法：\n" +
                                  "1.检查该NBT文件是否损坏。\n" +
                                  "2.确定该NBT文件使用常规压缩格式。\n" +
        ...
        {
            if (!ConfirmAction("打开 Minecraft 根目录吗？"))
                return;
            ...
            try {
                ...
                MessageBox.Show("无法打开默认 Minecraft 存档文件夹");
                ...
                try {
                    ...
                    MessageBox.Show("无法打开当前目录，此工具可能与您的运行平台不兼容。");
                    Console.WriteLine(e.Message);
                    Application.Exit();
                }
            }
        ...
        private bool ConfirmExit ()
        {
            if (_controller.CheckModifications()) {
                if (MessageBox.Show("你当前存在未保存的更改，要关闭吗QWQ?", "警告", MessageBoxButtons.OKCancel) != DialogResult.OK)
                    return false;
            }
        ...
        {
            if (_controller.CheckModifications()) {
                if (MessageBox.Show("你当前存在未保存的更改，要关闭吗QWQ?" + actionMessage, "警告", MessageBoxButtons.OKCancel) != DialogResult.OK)
                    return false;
            }
        ...
        {
            if (_controller.CheckModifications()) {
                if (MessageBox.Show("你当前存在未保存的更改，要关闭吗QWQ?" + e.Message, "警告", MessageBoxButtons.OKCancel) != DialogResult.OK)
                    e.Cancel = true;
            }
        }
        ...
        private void replaceToolStripMenuItem_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    MessageBox.Show("请先打开一个文件/文件夹并选取一个有效标签！",
                                  "提示",
                ...
                DataNode selectedDataNode = _nodeTree.SelectedNode.Tag as DataNode;
                if (selectedDataNode == null)
                {
                    MessageBox.Show("所选节点不包含有效数据，无法替换！",
                                  "错误",
                                  MessageBoxButtons.OK,
                ...
                using (Form form = new FindReplace(this, _controller, selectedDataNode))
                {
                    form.Show();
                }
            }
            ...
            {
                MessageBox.Show($"打开替换窗口时发生错误: {ex.Message}",
                              "错误",
                              MessageBoxButtons.OK,
                              MessageBoxIcon.Error);
            }
        ...
        private void findBlockToolStripMenuItem_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (!(_nodeTree.SelectedNode.Tag is DataNode selectedDataNode))
                {
                    MessageBox.Show("所选节点不包含有效数据！", "错误", MessageBoxButtons.OK, MessageBoxIcon.Error);
                    return;
                }

                using (FindBlock form = new FindBlock(selectedDataNode))
                {
                    if (form.ShowDialog() == DialogResult.OK)
                    {
                        if (form.Result != null)
                        ...
                        {
                            MessageBox.Show("未找到区块！", "提示", MessageBoxButtons.OK, MessageBoxIcon.Information);
                        }
                    }
                }
            }
            ...
            {
                MessageBox.Show($"搜索区块时发生错误: {ex.Message}", "错误", MessageBoxButtons.OK, MessageBoxIcon.Error);
            }
        }
